...
for i = 1:length(ModelForce)
	...
	switch forcename
		...
		case 'sinewave'
			force(i)=sin(dawn/(2*pi) + (2*t*pi)/CP)/2 + 1/2;
		case '60'
			force(i)=- (tanh(25*t - 90000)/2 + 1/2)*(tanh(25*t - 97500)/2 - 1/2) - (tanh(25*t - 180000)/2 + 1/2)*(tanh(25*t - 187500)/2 - 1/2) - (tanh(25*t - 270000)/2 + 1/2)*(tanh(25*t - 277500)/2 - 1/2) - (tanh(25*t)/2 + 1/2)*(tanh(25*t - 7500)/2 - 1/2);
		case '100'
			force(i)=- (tanh(25*t - 150000)/2 + 1/2)*(tanh(25*t - 157500)/2 - 1/2) - (tanh(25*t - 300000)/2 + 1/2)*(tanh(25*t - 307500)/2 - 1/2) - (tanh(25*t - 450000)/2 + 1/2)*(tanh(25*t - 457500)/2 - 1/2) - (tanh(25*t)/2 + 1/2)*(tanh(25*t - 7500)/2 - 1/2);
		case '200'
			force(i)=- (tanh(25*t - 300000)/2 + 1/2)*(tanh(25*t - 307500)/2 - 1/2) - (tanh(25*t - 600000)/2 + 1/2)*(tanh(25*t - 607500)/2 - 1/2) - (tanh(25*t - 900000)/2 + 1/2)*(tanh(25*t - 907500)/2 - 1/2) - (tanh(25*t)/2 + 1/2)*(tanh(25*t - 7500)/2 - 1/2);
		case 'cts'
			force(i)=1;
		case 'noforce'
			force(i)=0;
