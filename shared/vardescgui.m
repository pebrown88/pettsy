function r = vardescgui(model)

persistent newFig

%draw figure
newFig=figure('resize', 'off', 'menubar', 'none' ,'Name', [model.name ' Model Variables'] ,'NumberTitle','off');
...
tbl = uitable('units', 'centimeters', 'position', [0.5 0.5 pwidth-1 pheight-1], ...
    ...
    'columnname', {'Variable', 'Description'}, ...
    'parent', panel);

set(tbl, 'units', 'pixels');
tblwidth = get(tbl, 'position');
tblwidth = tblwidth(3);
if length(model.vnames) >= 18
    tblwidth = tblwidth*0.95;
end

set(tbl, 'data', [model.vnames model.vardesc], 'columnwidth', {tblwidth*0.265 tblwidth*0.6});
