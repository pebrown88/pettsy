function modelpath = mymake(name, varargin)
...
%%%%% GM:ADDED THE RATES_RHS
cd(DefsDir);
try
    rhs = feval(str2func([name '_model']), t, y, s); 
    oderatesrhs = feval(str2func([name '_oderates_sym']), t, y, s);
    s = [s, 'Omega', '=str2sym(''','Omega','''); '];
    ratesrhs = feval(str2func([name '_rates_sym']), t, y, s);
...
%now create equtations files
% keyboard
updatebar(wbHndl, inc, 'Installing the model. Saving model equations...');
%%%%% GM: savesystem changed to savesystemFI to get oderatesrhs % back to
%%%%% normal (I think) because oderatesrhs is moved to saveoderatesjac
savesystemFI(name, mdir, rhs, orbit_type, numForce);
savesystemFInoforce(name, mdir, rhs);

updatebar(wbHndl, 0, 'Installing the model. Saving model jacobian matrix...');
dydtdy_tmp = savejac(name, mdir1, wbHndl, inc, orbit_type, numForce);
dydtdy_tmpnoforce = savejacnoforce(name, mdir1, wbHndl, inc);

%%%%% GM: added saveoderatesjac to get oderates jac 
updatebar(wbHndl, 0, 'Installing the model. Saving rates jacobian matrix...');
% keyboard
doderatesdy_tmp = saveoderatesjac(name, oderatesrhs,mdir1, orbit_type, numForce);
dratesdy_tmp = saveratesjac(name, ratesrhs,mdir1, orbit_type, numForce);
...
%%%%% GM: added savedoderatesdpar to get oderates derivatives with respect to parameters 
updatebar(wbHndl, 0, 'Installing the model. Saving model parameter derrivatives...');
doderatesdk_tmp = savedoderatesdpar(name, mdir1, oderatesrhs, orbit_type, numForce);
dratesdk_tmp = savedratesdpar(name, mdir1, ratesrhs, orbit_type, numForce);
...
dydtdk_tmp = subs(dydtdk_tmp, vari, varnames);
dydtdy_tmp = subs(dydtdy_tmp, [varsym pari forcei], [varnames parn' forcesym]);
dydtdy_tmpnoforce = subs(dydtdy_tmpnoforce, [varsym pari forcei], [varnames parn' forcesym]);
doderatesdk_tmp = subs(doderatesdk_tmp, vari, varnames);
dratesdk_tmp = subs(dratesdk_tmp, vari, varnames);
doderatesdy_tmp = subs(doderatesdy_tmp, [varsym pari forcei], [varnames parn' forcesym]);
dratesdy_tmp = subs(dratesdy_tmp, [varsym pari forcei], [varnames parn' forcesym]);
...
for i = 1:dim
  ...
  for j = 1:dim
      dydtdynof{i,j} = char(dydtdy_tmpnoforce(i,j));
  end
...
% keyboard
doderatesdk = cell(size(doderatesdk_tmp,1),size(doderatesdk_tmp,2)) ;
dratesdk = cell(size(dratesdk_tmp,1),size(dratesdk_tmp,2)) ;
doderatesdy = cell(size(doderatesdk_tmp,1),size(doderatesdy_tmp,2)) ;
dratesdy = cell(size(dratesdk_tmp,1),size(dratesdy_tmp,2)) ;
for i=1:size(doderatesdk_tmp,1)
    for j=1:size(doderatesdk_tmp,2)
        doderatesdk{i,j} = char(doderatesdk_tmp(i, j));%note transpose required here so matrix matches model_dp file
    end
    for j=1:size(dratesdk_tmp,2)
        dratesdk{i,j} = char(dratesdk_tmp(i, j));%note transpose required here so matrix matches model_dp file
    end
    for j=1:size(doderatesdy_tmp,2)
        doderatesdy{i,j} = char(doderatesdy_tmp(i,j));
    end
    for j=1:size(dratesdy_tmp,2)
        dratesdy{i,j} = char(dratesdy_tmp(i,j));
    end
end

save(fullfile(mdir1, [name '_eqn_info.mat']), 'eqn_info', 'model_odes', 'dydtdy', 'dydtdk','doderatesdk','doderatesdy', 'parn', 'varnames', 'forcenames', 'dforcesym');
% keyboard
if CreateSecond
    ...
    savehes(name, mdir2, rhs, orbit_type, numForce);
    savehes1(name, mdir2, rhs, orbit_type, numForce);
